...
The YouTube dataset loads from the published Marine Chariot Google Sheet CSV by default and refreshes in the browser every five minutes while the tab is active. If the live sheet cannot be reached, the app falls back to `marine-ch-data.csv` and labels the active source in the dashboard header.
