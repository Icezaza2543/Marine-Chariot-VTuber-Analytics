        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 relative z-10 flex flex-col md:flex-row justify-between items-center md:items-start gap-8">
          <div className="text-center md:text-left flex-1 max-w-lg">
            ...
            <p className="text-sm text-[var(--mc-text-muted)] mb-1">
              ...
              {xFetchedAt
                ? ` · X cache ${formatThaiDate(xFetchedAt)}`
                : ' · X cache ยังไม่ถูก fetch'}
            </p>
            <p className="text-xs text-slate-500 opacity-80">
              โปรเจกต์ fan-made ไม่เกี่ยวข้องกับเจ้าของช่องอย่างเป็นทางการ ·
              เจ้าของข้อมูลและคอนเทนต์: Marine Chariot
            </p>
            <p className="text-xs text-slate-500 opacity-80">
              โค้ดอยู่ภายใต้ MIT License; สิทธิ์นี้ไม่ครอบคลุมข้อมูลช่อง แบรนด์ วิดีโอ โพสต์
              หรือสื่อของ Marine Chariot
            </p>
          </div>

          <div className="flex flex-col items-center md:items-end gap-3">
            <nav
              aria-label="ลิงก์โปรเจกต์"
              className="flex items-center gap-4 text-sm text-[var(--mc-text-muted)] font-medium"
            >
              <a
                href={project.githubUrl}
                rel="noreferrer"
                target="_blank"
                className="hover:text-[var(--mc-cyan)] transition-colors"
              >
                GitHub
              </a>
              <button
